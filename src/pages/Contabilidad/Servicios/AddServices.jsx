...
const AddServices = () => {
  const handleSubmit = async (e) => {
     ...
     */
    try {
      const response = await axios.post("https://veterinariamap6iv6-production.up.railway.app/api/v1/catalogo/servicioC", {
        nombre_ser: e.target.serviceName.value,
        descripcion_ser: e.target.serviceDesc.value
      }, {
        headers: {
          ...
          <tr>
            <th className='p-2'>
              <div className='w-2/3 rounded-full border-4 border-[#6ED4A5] bg-[#6ED4A5]'>
                <button className='p-2' type="submit" onClick={redireccion => { window.location.href = "/GastosFijos" }}>
